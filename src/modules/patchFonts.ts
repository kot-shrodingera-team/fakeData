...
const defineobjectproperty = (val: any, e: boolean, c = true, w = true) => {
...
const patchFonts = (
  ...
  const fontListToUse = fakeProfile.Fonts.map((x) => x.toLowerCase());

  const baseFonts = ['auto'];
  const keywords = ['inherit', 'auto'];
  baseFonts.push(...fontListToUse);
  baseFonts.push(...keywords);

  const getAllowedFontFamily = (family: string) => {
    const fonts = family.replace(/"|'/g, '').split(',');

    const allowedFonts = fonts.filter((font) => {
      if (font && font.length) {
  ...
  function modifiedCssSetProperty(key: string, val: string) {
  ...
  function makeModifiedSetCssText(
    originalSetCssText: (...args: unknown[]) => unknown
  ) {
    return (css: string) => {
  ...
  const recursivelyModifyFonts = (elem: HTMLElement) => {
    if (elem) {
  ...
  const makeModifiedInnerHtml = (originalInnerHtml: () => unknown) => {
    return function modifiedInnerHtml(html: HTMLElement) {
  ...
  function modifiedAppend(child: HTMLElement) {
    try {
      // eslint-disable-next-line no-param-reassign
  ...
  const overrideFunc = (obj: any, name: string, f: any) => {
    Object.defineProperty(obj.prototype, name, defineobjectproperty(f, true));
    // eslint-disable-next-line no-param-reassign
    obj.prototype[name].toString = () => `${name}() { [native code] }`;
    // eslint-disable-next-line no-param-reassign
    obj.prototype[name].toString.toString = () =>
      'toString() { [native code] }';
  };

  const overrideSetter = (obj: any, name: string, makeSetter: any) => {
